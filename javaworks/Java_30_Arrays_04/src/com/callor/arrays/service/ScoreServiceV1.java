...
public class ScoreServiceV1 {
	...
	/*
	 * 클래스 영역에 선언된 변수
	 * SocreServiceV1 클래스에 선언된 모든 method 에서
	 * 변수에 접근(읽고, 저장) 할 수 있다.
	 * 
	 * 인스턴스 변수 :
	...
	public ScoreServiceV1() {
		intKor = new int[100];	// intKor 배열이 사용(저장, 읽기)할 준비가 된다.		
	}
	
	public void makeScore() {
		for(int i = 0 ; i < intKor.length ; i++) {
			/*
			 * random() * 50 을 연산하면 0부터 49까지 범위의 임의 수가 생선된다
	...
	public void scoreprint() {
		...
		/*
		 * Line 크랠스에 선언된 dLine 속성(변수)는 static이기 때문에 
		 * Line클래스를 line 인스턴스로 만들고 line.dLine 처럼 접근할 필요가 없다.
		 * Line line = new Line();

		/*
		 * dLine 속성(변수)은 static 으로 선언되어 있기 때문ㅇ
		 * Line 클래스를 인스턴스로 생성하지 않고도 접근할 수 있다.
		 */
		System.out.println(Line.dLine);
		System.out.println("국어 성절 일람표");
