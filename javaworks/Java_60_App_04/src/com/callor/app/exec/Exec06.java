...
public class Exec06 {
	...
	public static void main(String[] args) {
		 ...
		 * index 와 나머지 연산을 수행하여 0나오면 break
		 * 		  =>소수가 아니다
		 * 각 요소값이 소수이려면 index 와 나머지 연산에서
		 *   		한번도 0이 나오면 안된다
		 *   		즉 if 조건이 한번도 true 가 아니어야하고
		 *    		break 가 실행되지 않아야 한다.
		...
		for(int j = 0; j <nums.length ; j++) {
			...
			for(index = 2 ; index < nums[j] ; index++) {
				// 9 % 2 는 나머지가 1, 9 % 3 는 나머지가 0이 된다.
				// 9 % 3 이 0이 되는 순가 break 를 만나서 for가 중단 
				if(nums[j] % index == 0) {
					break;
				}
			}	// for(index) end
			...
			// true 가 된다는 것을 break를 만나지 않았다는 것이다.
			if(nums[j] <= index)
			System.out.println(nums[j]+"는 소수입니다.");
