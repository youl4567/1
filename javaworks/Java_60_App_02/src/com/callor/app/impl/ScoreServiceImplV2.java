		...
		while(true) {
			...
			for(index = 0 ; index < strSubject.length ; index++) {
				...
				if(intScore[index] == null) {
					 ...
					 * 점수를 숫자로 입력하지 않고 엔터만 누르거나
					 * 다른 문자열이 섞인 경우라면
					 * "국어점수는 정수만 입력" 메세지를 보여주고  null을 리턴할 것이다.
					 * 그러면 index 값을 1 감소시켜 -1로 만든 후
					 * 다시 for() 처음으로 점프한다.
					 * for(index = -1; .... ) {} 가 다시 실행될 것이다.
					 * for( ;; index++) 에 의해서 index 는 다시 0이된다.
